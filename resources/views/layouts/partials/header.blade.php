<header class="sticky top-0 z-40 bg-forest-900 border-b border-forest-800 shadow-md text-cream-100" x-data="{ mobileMenuOpen: false }">
    <div class="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <!-- Logo (Fixed flex-shrink-0 to prevent overlap) -->
        <a href="{{ route('home') }}" class="flex items-center space-x-2.5 flex-shrink-0 mr-3 xl:mr-6">
            <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-amberAcc-500 text-forest-950 flex items-center justify-center font-extrabold shadow-md flex-shrink-0">
                <i class="fa-solid fa-bolt text-lg sm:text-xl"></i>
            </div>
            <div class="flex-shrink-0">
                <span class="text-lg sm:text-2xl font-black tracking-tight text-white block leading-none">E-BIKE <span class="text-amberAcc-500">4 U</span></span>
                <span class="block text-[8px] sm:text-[10px] uppercase font-bold tracking-widest text-cream-200/70 mt-0.5">UK Sales & Rentals</span>
            </div>
        </a>

        <!-- Desktop Navigation Menu (Responsive spacing and font sizing) -->
        <nav class="hidden xl:flex items-center space-x-5 2xl:space-x-7 font-bold text-cream-100 text-xs 2xl:text-sm flex-shrink-0">
            <a href="{{ route('home') }}" class="hover:text-amberAcc-500 transition-colors border-b-2 py-1 {{ request()->routeIs('home') ? 'border-amberAcc-500 text-amberAcc-500' : 'border-transparent' }}">Home</a>
            <a href="{{ route('catalog.index', ['type' => 'ebike']) }}" class="hover:text-amberAcc-500 transition-colors py-1">E-Bikes</a>
            <a href="{{ route('catalog.index', ['type' => 'rental']) }}" class="hover:text-amberAcc-500 transition-colors text-amberAcc-500 font-extrabold flex items-center py-1">
                <span class="w-2 h-2 rounded-full bg-amberAcc-500 animate-pulse mr-1.5"></span> Bike Rental
            </a>
            <a href="{{ route('catalog.index', ['type' => 'accessory']) }}" class="hover:text-amberAcc-500 transition-colors py-1">Accessories</a>
            
            <!-- Categories Dropdown -->
            <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                <button class="hover:text-amberAcc-500 transition-colors flex items-center py-1">
                    Categories <i class="fa-solid fa-chevron-down text-[10px] ml-1 text-cream-300"></i>
                </button>
            ...
            <a href="{{ route('cms.faqs') }}" class="hover:text-amberAcc-500 transition-colors py-1">FAQs</a>
            <a href="{{ route('cms.contact') }}" class="hover:text-amberAcc-500 transition-colors py-1">Contact</a>
        </nav>

        <!-- Desktop Search Bar -->
        <form action="{{ route('catalog.index') }}" method="GET" class="hidden xl:flex items-center flex-1 max-w-xs mx-4 2xl:mx-6 flex-shrink">
            <div class="relative w-full flex items-center bg-forest-950 border border-cream-200/30 rounded-full pl-4 pr-1 py-1">
                ...
                <button type="submit" class="w-8 h-8 rounded-full bg-white text-forest-950 flex items-center justify-center hover:bg-amberAcc-500 transition-colors flex-shrink-0">
        ...
        <div class="flex items-center space-x-2.5 sm:space-x-3.5 flex-shrink-0">
            <!-- Wishlist Icon -->
            <a href="{{ route('customer.wishlist') }}" class="relative text-cream-100 hover:text-amberAcc-500 transition-colors p-1.5" title="Wishlist">
            ...
            @auth
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center space-x-1.5 border border-cream-200/30 rounded-full py-1 px-2.5 hover:bg-forest-800 transition-colors">
                        ...
                        <span class="text-xs font-bold text-cream-100 hidden 2xl:inline max-w-[100px] truncate">{{ auth()->user()->name }}</span>
            ...
            @else
                <a href="{{ route('login') }}" class="text-xs font-black text-forest-950 bg-amberAcc-500 hover:bg-amberAcc-400 rounded-full px-3 py-1.5 sm:px-4 sm:py-2 shadow-md transition-all flex items-center">
                    <i class="fa-solid fa-user mr-1"></i> <span class="hidden sm:inline">Account</span>
                </a>
            @endauth

            <!-- Mobile Hamburger Button -->
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="xl:hidden text-cream-100 text-xl p-1.5 focus:outline-none">
    ...
    <div x-show="mobileMenuOpen" x-transition x-cloak class="xl:hidden bg-forest-950 text-cream-100 border-b border-forest-800 px-5 py-5 space-y-4 shadow-2xl">
        <!-- Mobile Search Input -->
        <form action="{{ route('catalog.index') }}" method="GET" class="flex items-center bg-forest-900 border border-cream-200/30 rounded-full pl-4 pr-1 py-1">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Search E-Bikes, accessories..." 
                   class="w-full bg-transparent text-cream-100 placeholder-cream-300/60 text-xs focus:outline-none font-medium">
            <button type="submit" class="w-8 h-8 rounded-full bg-amberAcc-500 text-forest-950 flex items-center justify-center flex-shrink-0">
                <i class="fa-solid fa-magnifying-glass text-xs"></i>
            </button>
        </form>
