<header class="sticky top-0 z-40 bg-forest-900 border-b border-forest-800 shadow-md text-cream-100" x-data="{ mobileMenuOpen: false }">
    <div class="container mx-auto px-4 py-3.5 flex items-center justify-between">
        ...
        <a href="{{ route('home') }}" class="flex items-center space-x-2.5">
            ...
            <div>
                <span class="text-2xl font-black tracking-tight text-white">E-BIKE <span class="text-amberAcc-500">4 U</span></span>
                <span class="block text-[10px] uppercase font-bold tracking-widest text-cream-200/70 -mt-1">UK Sales & Rentals</span>
            </div>
        </a>
        ...
        <div class="flex items-center space-x-4">
            <!-- Wishlist Icon -->
            <a href="{{ route('customer.wishlist') }}" class="relative text-cream-100 hover:text-amberAcc-500 transition-colors p-2" title="Wishlist">
                <i class="fa-regular fa-heart text-lg"></i>
            </a>

            <!-- Cart Trigger Button with Circle Counter Badge -->
            <button @click="isCartOpen = true" class="relative flex items-center space-x-1.5 bg-forest-950/80 hover:bg-forest-950 border border-cream-200/20 px-3.5 py-1.5 rounded-full text-xs font-bold transition-all">
                <span>Cart</span>
            ...
            @auth
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center space-x-2 border border-cream-200/30 rounded-full py-1 px-3 hover:bg-forest-800 transition-colors">
                        <img src="{{ auth()->user()->avatar ?? 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=80' }}" class="w-7 h-7 rounded-full object-cover">
            ...
            @else
                <a href="{{ route('login') }}" class="text-xs font-black text-forest-950 bg-amberAcc-500 hover:bg-amberAcc-400 rounded-full px-4 py-2 shadow-md transition-all flex items-center">
                    <i class="fa-solid fa-user mr-1.5"></i> Account
                </a>
            @endauth

            <!-- Mobile Hamburger Button -->
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden text-cream-100 text-xl p-2 focus:outline-none">
    ...
    <!-- Mobile Drawer Menu -->
    <div x-show="mobileMenuOpen" x-transition x-cloak class="lg:hidden bg-forest-950 text-cream-100 border-b border-forest-800 px-4 py-4 space-y-3">
        <a href="{{ route('home') }}" class="block text-sm font-bold">Home</a>
        <a href="{{ route('catalog.index', ['type' => 'ebike']) }}" class="block text-sm font-bold">E-Bikes</a>
        <a href="{{ route('catalog.index', ['type' => 'rental']) }}" class="block text-sm font-bold text-amberAcc-500">Bike Rental</a>
        <a href="{{ route('catalog.index', ['type' => 'accessory']) }}" class="block text-sm font-bold">Accessories</a>
        <a href="{{ route('cms.faqs') }}" class="block text-sm font-bold">FAQs</a>
        <a href="{{ route('cms.contact') }}" class="block text-sm font-bold">Contact Us</a>
    </div>
</header>
