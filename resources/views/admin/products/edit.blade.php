...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-3xl border border-slate-200 shadow-xs space-y-6">
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" 
          x-data="productEditForm()" @submit="handleSubmit($event)" class="space-y-6 text-xs">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="type" x-model="type" required class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold text-slate-900 focus:ring-2 focus:ring-brandOrange-500">
                    <option value="ebike" {{ old('type', $product->type) == 'ebike' ? 'selected' : '' }}>E-Bike</option>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" x-model="price" @input="validatePrices()" required class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-black text-slate-900 focus:ring-2 focus:ring-brandOrange-500">
            </div>

            <!-- Discount Price with Frontend Validation -->
            <div>
                <label class="block font-bold text-slate-700 mb-1">Discount Price (£) (Optional)</label>
                <input type="number" step="0.01" name="discount_price" x-model="discountPrice" @input="validatePrices()" class="w-full bg-slate-50 border rounded-xl p-3 font-bold text-slate-900 focus:ring-2 focus:ring-brandOrange-500" :class="priceError ? 'border-rose-500 bg-rose-50/50' : 'border-slate-200 bg-slate-50'">
        ...
        <div x-show="type === 'ebike'" x-cloak class="bg-slate-50 p-6 rounded-2xl border border-slate-200 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="rental_price_weekly" value="{{ old('rental_price_weekly', $product->rental_price_weekly) }}" placeholder="180.00" class="w-full bg-white border border-slate-200 rounded-xl p-3 font-bold text-slate-900">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Security Deposit (£) (Default: £250.00)</label>
                    <input type="number" step="0.01" name="rental_security_deposit" value="{{ old('rental_security_deposit', $product->rental_security_deposit ?? '250.00') }}" placeholder="250.00" class="w-full bg-white border border-slate-200 rounded-xl p-3 font-bold text-slate-900">
                </div>
            </div>
        </div>

        <!-- E-Bike Specifications (Shown ONLY for E-Bikes) -->
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" :disabled="priceError !== ''" class="py-3.5 px-7 bg-brandOrange-500 hover:bg-brandOrange-600 disabled:bg-slate-300 text-white font-black rounded-xl shadow-md uppercase">Update Product</button>
        </div>
    </form>
