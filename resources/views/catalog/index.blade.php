    ...
    <div class="flex flex-col lg:flex-row gap-6 lg:gap-8">
        ...
        <div x-show="mobileFilterOpen || isDesktop" x-cloak class="w-full lg:w-64 flex-shrink-0 bg-white p-5 md:p-6 rounded-3xl border border-cream-200 shadow-sm h-fit">
            ...
            <form @change="filterProducts()" id="filterForm" class="space-y-6">
                <!-- Type Filter -->
                <div>
                    <label class="block text-xs font-black text-forest-900 uppercase tracking-wider mb-2">Category Type</label>
                    <div class="space-y-2 text-xs font-semibold">
                        <label class="flex items-center space-x-2 text-slate-700 cursor-pointer hover:text-amberAcc-600">
                            <input type="radio" name="type" value="" {{ request('type') == '' ? 'checked' : '' }} class="text-amberAcc-600 focus:ring-amberAcc-500">
                            <span>All Products</span>
                        </label>
                        <label class="flex items-center space-x-2 text-slate-700 cursor-pointer hover:text-amberAcc-600">
                            <input type="radio" name="type" value="ebike" {{ request('type') == 'ebike' ? 'checked' : '' }} class="text-amberAcc-600 focus:ring-amberAcc-500">
                            <span>E-Bikes Only</span>
                        </label>
                        <label class="flex items-center space-x-2 text-amberAcc-700 font-extrabold cursor-pointer">
                            <input type="radio" name="type" value="rental" {{ request('type') == 'rental' ? 'checked' : '' }} class="text-amberAcc-600 focus:ring-amberAcc-500">
                            <span>Rental Eligible E-Bikes</span>
                        </label>
                        <label class="flex items-center space-x-2 text-slate-700 cursor-pointer hover:text-amberAcc-600">
                            <input type="radio" name="type" value="accessory" {{ request('type') == 'accessory' ? 'checked' : '' }} class="text-amberAcc-600 focus:ring-amberAcc-500">
